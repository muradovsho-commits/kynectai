    ...
    <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 14, padding: '16px 20px', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 100, background: '#fef3c7', color: '#92400e' }}>{visible.length}</span>
        </div>
        ...
        {visible.slice(0, 5).map(r => (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 10, background: '#fef3c7', border: '1px solid #fde68a' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 12 }}>
              {onOpenContact && <button type="button" onClick={() => onOpenContact(r.contactId)} style={{ fontSize: 11, fontWeight: 600, padding: '5px 12px', borderRadius: 6, background: 'rgba(255,255,255,.7)', border: '1px solid #fde68a', color: '#92400e', cursor: 'pointer', fontFamily: 'Sora, sans-serif' }}>Open</button>}
              <button type="button" onClick={() => dismiss(r.id)} style={{ fontSize: 11, fontWeight: 600, padding: '5px 12px', borderRadius: 6, background: 'none', border: '1px solid #fde68a', color: '#92400e', cursor: 'pointer', fontFamily: 'Sora, sans-serif', opacity: 0.6 }}>Dismiss</button>
            </div>
          </div>
        ))}
