...
<p><em>By Series B, the founders have diluted from 85% to ~38%&mdash;but their shares are now worth ~38% × $175M = $66.5M, up from a nominal value near zero at founding.</em></p>
</div>`,
  },
  {
    title: 'The Option Pool Shuffle',
    content: `<p>Before each priced round, investors typically require that the option pool be sized (or "refreshed") to a target percentage of post-money shares, but the dilution from creating the pool comes out of the <em>pre-money</em> valuation. This is the "option pool shuffle"&mdash;it effectively reduces the true pre-money valuation because existing shareholders (founders and prior investors) absorb the dilution, not the new investor.</p>
...
<p>Dilution is not inherently bad. If a founder goes from 50% ownership of a $10M company ($5M) to 35% ownership of a $100M company ($35M), they've been "diluted" but are 7x richer. Dilution is destructive only when the valuation doesn't increase enough to offset the ownership reduction&mdash;which happens in flat or down rounds.</p>`,
  },
];
