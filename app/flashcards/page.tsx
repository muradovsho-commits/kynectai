    ...
    try {
      ...
      if (data.error) { setAiHistory([...newHistory, { role: 'assistant', content: 'Something went wrong — try again.' }]); }
        ...
        if (data.score) {
          ...
          const np = { ...perf };
          np.seen++;
          if (verdict === 'pass') np.pass++;
          else if (verdict === 'partial') np.partial++;
          else np.fail++;
              ...
              <>
                <div className="flash-card-container">
                  <div className="flash-card-single">
                    ...
                    {mode === 'practice' && (
                      <>
                        <button className={`flash-show-btn${showAnswer ? ' shown' : ''}`} onClick={() => setShowAnswer(!showAnswer)} type="button">
