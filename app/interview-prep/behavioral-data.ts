// Investment Banking Technical Mastery Manual — Module 9: Behavioral & Storytelling
export const BEHAVIORAL_SECTIONS = [
  {
    title: '9.1 Why Behavioral Matters in Banking',
    content: `<p>Technical skill gets you into the conversation. Behavioral fit often determines whether you get the offer.</p>
<p><strong>Bankers are evaluating:</strong> motivation, maturity, polish, communication, resilience, work ethic, self-awareness, whether they want to work with you at 2 a.m.</p>`,
  },
  {
    title: '9.2 "Tell Me About Yourself"',
    content: `<p>A strong answer should be concise, chronological enough to follow, and clearly directed toward banking.</p>
<p><strong>Good structure:</strong></p>
<ol><li>Present</li><li>Relevant past experiences</li><li>Why they led you to banking</li><li>Why now / why this role</li></ol>
<p><strong>Example structure:</strong></p>
<p>"I'm currently a junior studying finance and economics, and over the last two years I've become increasingly interested in how strategic and financial decisions drive business outcomes. That started with my coursework in accounting and valuation, but it became much more real through my internship experience, where I saw how transactions affect companies and investors. I especially liked the combination of analytical intensity, high stakes, and team-driven execution. That's what pushed me toward investment banking, and I'm now looking for the opportunity to build on that foundation in a demanding environment where I can contribute and learn quickly."</p>`,
  },
  {
    title: '9.3 Why Investment Banking',
    content: `<p>This answer must sound real, not copied.</p>
<p><strong>Strong components:</strong> attraction to transactions / strategic finance, interest in steep learning curve, desire for rigorous analytical training, team-based, high-performance environment, exposure to companies / industries / executives.</p>
<p><strong>Weak answer:</strong> "I like finance, I'm hardworking, and I want to learn a lot."</p>
<p><strong>Stronger answer:</strong> "I'm drawn to investment banking because it sits at the intersection of valuation, strategy, and execution. I like work that is analytically demanding and high stakes, and I've found through past experiences that I'm most engaged when I'm working on problems with real business consequences under tight deadlines. Banking also offers the chance to build an unusually strong foundation in corporate finance and transactions early in my career."</p>`,
  },
  {
    title: '9.4–9.5 Why Our Firm & Resume Walkthrough',
    content: `<h3>9.4 Why Our Firm</h3>
<p>This answer should include: the platform or franchise, the culture, group-specific reasons if relevant, evidence you have done real homework.</p>
<p>Avoid generic praise.</p>

<h3>9.5 Resume Walkthrough</h3>
<p>Your resume walkthrough should not be a list of bullets. It should be a narrative.</p>
<p><strong>Good flow:</strong> start point, key experiences, what each taught you, why they point naturally to banking.</p>
<p><strong>What bankers want:</strong> A candidate who looks intentional, self-aware, and upward-trending.</p>`,
  },
  {
    title: '9.6–9.8 Leadership Stories, Failure/Weakness, Behavior Under Pressure',
    content: `<h3>9.6 Leadership Stories</h3>
<p>Even though banking is not consulting, leadership still matters.</p>
<p><strong>Use stories that show:</strong> ownership, initiative, reliability under pressure, influence without authority, persistence.</p>

<h3>9.7 Failure / Weakness Questions</h3>
<p><strong>Good failure answer ingredients:</strong> real setback, ownership, what changed after, evidence of improvement.</p>
<p><strong>Good weakness answer ingredients:</strong> real but manageable weakness, concrete steps taken, honest self-awareness.</p>
<p>Do not give fake strengths disguised as weaknesses.</p>

<h3>9.8 Behavior Under Pressure</h3>
<p>Bankers care whether you can handle stress without becoming sloppy, negative, or fragile.</p>
<p><strong>Good stories often involve:</strong> tough deadlines, competing demands, limited information, execution pressure, team coordination.</p>`,
  },
  {
    title: '9.9–9.10 Behavioral Story Bank & How to Sound Strong + Module 9 Practice',
    content: `<h3>9.9 Behavioral Story Bank</h3>
<p><strong>Prepare stories for:</strong> leadership, teamwork, conflict, failure, success, working under pressure, persuading someone, attention to detail, ethical judgment.</p>
<p><strong>For each story, know:</strong> setup, challenge, your role, actions, result, lesson.</p>

<h3>9.10 How to Sound Like a Strong Candidate</h3>
<p><strong>Strong candidates are:</strong> concise, specific, reflective, energetic but controlled, not robotic.</p>
<p>Bankers want confidence without rehearsed artificiality.</p>

<h3>Module 9 Practice Drills</h3>
<p><strong>Prepare and practice:</strong></p>
<ol><li>Tell me about yourself</li><li>Why investment banking?</li><li>Why our firm?</li><li>Walk me through your resume</li><li>Tell me about a time you worked under pressure</li><li>Tell me about a failure</li><li>Tell me about a conflict</li></ol>
<p><strong>Explain-out-loud exercise:</strong> Record yourself giving your "Why banking?" answer three times and refine it for: specificity, credibility, energy, concision.</p>
<p><strong>Self-testing framework — For every behavioral answer, ask:</strong></p>
<ol><li>Is it specific?</li><li>Is my role clear?</li><li>Is there real difficulty or stakes?</li><li>Do I show what I learned?</li><li>Does it connect to the job?</li></ol>`,
  },
];
