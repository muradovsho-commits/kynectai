// Investment Banking Technical Mastery Manual — Module 8: Technical Interview Flashcards
...
const toCards = (questions: string[], section: string): Card[] =>
  questions.map((question) => ({ question, answer: answerForQuestion(question, section) }));

const renderCards = (cards: Card[], startIndex: number): string =>
  cards
    .map(
      (card, idx) =>
        `<h3>Question ${startIndex + idx}: ${escapeHtml(card.question)}</h3><p><strong>Strong answer:</strong> ${escapeHtml(card.answer)}</p>`
    )
    .join('');

const accountingCards = toCards(accountingQuestions, 'Accounting & Financial Statements (Q1-Q80)');
const valuationCards = toCards(valuationQuestions, 'Valuation & DCF (Q81-Q170)');
const maCards = toCards(maQuestions, 'M&A Concepts & Merger Models (Q171-Q260)');
const lboCards = toCards(lboQuestions, 'LBO & Financial Sponsors (Q261-Q340)');
const capitalMarketsCards = toCards(capitalMarketsQuestions, 'Capital Markets & Macro (Q341-Q410)');
const behavioralCards = toCards(behavioralQuestions, 'Behavioral & Fit (Q411-Q500)');

export const MARKETS_SECTIONS = [
  {
    title: '8.1 Accounting & Financial Statements (Q1-Q80)',
    content: `<p>Investment Banking flashcards: all accounting and financial statements questions were replaced with your uploaded set. Each card now includes a structured strong answer.</p>${renderCards(accountingCards, 1)}`,
  },
  {
    title: '8.2 Valuation & DCF (Q81-Q170)',
    content: `<p>These flashcards focus on intrinsic and market-based valuation, DCF mechanics, and advanced valuation edge cases.</p>${renderCards(valuationCards, 81)}`,
  },
  {
    title: '8.3 M&A Concepts & Merger Models (Q171-Q260)',
    content: `<p>These flashcards cover strategic rationale, merger model mechanics, deal structure, purchase accounting, and execution risk.</p>${renderCards(maCards, 171)}`,
  },
  {
    title: '8.4 LBO & Financial Sponsors (Q261-Q340)',
    content: `<p>These flashcards cover LBO mechanics, capital structure, debt instruments, returns, and sponsor value creation.</p>${renderCards(lboCards, 261)}`,
  },
  {
    title: '8.5 Capital Markets & Macro + Behavioral & Fit (Q341-Q500)',
    content: `<h3>Capital Markets & Macro (Q341-Q410)</h3>${renderCards(capitalMarketsCards, 341)}<h3>Behavioral & Fit - IB Focused (Q411-Q500)</h3>${renderCards(behavioralCards, 411)}`,
  },
];
