...
<p><strong>Taxes:</strong> Apply the tax rate to Operating Income (EBIT) to get NOPAT (Net Operating Profit After Tax). This is a "theoretical" tax because UFCF ignores interest expense&mdash;you're calculating the tax burden as if the company had no Debt.</p>
...
<div class="answer">A DCF provides an intrinsic estimate based on the company's own projected cash flows&mdash;it's independent of market sentiment but highly sensitive to assumptions. Comps provide a market-based estimate anchored to how investors currently price similar companies&mdash;it's grounded in observable data but assumes the market is pricing those companies correctly. Using both lets you cross-check: if they broadly agree, you have higher confidence. If they disagree significantly, it highlights either a modeling assumption worth questioning or a market mispricing worth investigating.</div>
</div>
