...
const IB_1: Scenario = {
  ...
  opening: [
    ...
    { personaId: 'vp', text: `When you get the comps to David I'll review immediately. Quick reminders: peers defensible, multiples reconcile to source, EV is market cap plus net debt — no shortcuts. Clean format. Source footnoted. Also: after the comps I'll need the one-pager teaser for Lighthouse — company overview, growth metrics, why-now, three investment highlights. We can talk about that after I see the comps.` },
  ],
  artifacts: [
    {
      id: 'comp-sheet',
...
const IB_2: Scenario = {
  ...
  opening: [
    { personaId: 'md', text: `Listen — I'm getting on a plane in twenty minutes. The cardiovascular regulatory risk is bigger than we've been modeling. I want the bear case to reflect a 15-20% revenue haircut on that segment for three years before recovery. Rebuild the DCF, then update the football field so the bear scenario actually shows up visually. I want to see both files before the 8am call tomorrow.` },
    { personaId: 'vp', text: `Marcus's framing is right. When you rebuild: don't just lower the growth rate uniformly — segment it. Cardiovascular gets the haircut, other segments stay on plan. Show me bear / base / bull on the DCF tab clearly labeled. Then update the FF in PowerPoint with revised ranges. Send me the Excel first, I'll review while you work on the slide.` },
  ],
  artifacts: [
    {
      id: 'dcf-rebuild',
...
const IB_3: Scenario = {
  ...
  opening: [
    { personaId: 'assoc', text: `Welcome to the desk. Two things by EOD — comp set on Project Atlas (peers: PCTY, PAYC, ZD, BSY, NICE, AZPN; EV/Rev and EV/EBITDA NTM) and a paper LBO. LBO assumptions: sponsor pays 9.0x LTM EBITDA on $12M, 50% debt at SOFR+500 (assume SOFR = 5.0%), 5-year hold, exit at the same multiple, 10% revenue CAGR, 200bps margin expansion linearly. Show me MOIC and IRR. Don't overthink, just get the math right. Send them as separate files.` },
  ],
  artifacts: [
    {
      id: 'comp-set',
...
const PE_1: Scenario = {
  ...
  opening: [
    { personaId: 'sr-assoc', text: `CIM for Project Summit is in the deal folder — DTC outdoor apparel, $180M revenue, $35M EBITDA. Sam wants a quick paper LBO and a one-pager by EOD. Assume 10.5x entry, 55% debt at SOFR+475 (SOFR = 5.0%), 5-yr hold, exit at entry multiple. Base case: 8% revenue CAGR, hold EBITDA margin at 19%. Then show me a downside (5% revenue, margin to 17%) and an upside (12% revenue, margin to 21%). MOIC and IRR on each. One-pager is your call on what to flag — Sam reads them fast, lead with the answer.` },
    { personaId: 'principal', text: `Don't make this complicated. I just want to know: at 10.5x, does this clear our hurdle in any reasonable case? And what's the one thing that could break it? Send when ready.` },
  ],
  artifacts: [
    {
      id: 'paper-lbo',
...
const PE_2: Scenario = {
  ...
  opening: [
    { personaId: 'principal', text: `IC is Tuesday. Diane is going to grill us on risks. I need you to draft the Key Risks & Mitigants section of the IC memo — 4-6 risks, each with a real mitigant (not just "we'll watch this"). I also want you ready for two specific questions: (1) what's our edge vs. a strategic acquirer, (2) bear case on Meta/Google dependency since 65% of customer acquisition is paid social/search.` },
...
const PE_3: Scenario = {
  ...
  opening: [
    { personaId: 'sr-assoc', text: `Tomorrow's mgmt presentation. I need two things: a structured question list (15-20 Qs across financial, commercial, organizational, strategic) and a clean takeaways template we'll fill in live. Question list goes in a Word doc — group by category. Make the questions specific — "tell us about growth" is a wasted question. Send when ready.` },
  ],
  artifacts: [
    {
      id: 'question-list',
...
const CON_1: Scenario = {
  ...
  opening: [
    { personaId: 'em', text: `You're owning the margin diagnostic for NorthPeak. Two deliverables by EOD: (1) issue tree — what are all the possible drivers of the 400bps compression, structured MECE; (2) a single slide that lays out the top 3 drivers with rough sizing of each. The data you need is in the case folder. Marcus will look at both tomorrow morning.` },
...
const CON_2: Scenario = {
  ...
  opening: [
    { personaId: 'em', text: `Tomorrow you're interviewing Tom Reilly, VP of Ops at NorthPeak. He's not a fan of consultants. We have 60 minutes. I need an interview guide: opening framing (2 min — why we're here, what we hope to learn, what we'll share back), then 8-12 structured questions across pack mix, channel push, supply chain, and ops capability. Make the questions specific — generic ones will tank the meeting. Write it as a .docx — group by topic with rough time allocations.` },
  ],
  artifacts: [
    {
      id: 'interview-guide',
...
const CON_3: Scenario = {
  ...
  opening: [
    { personaId: 'partner', text: `Readout's tomorrow. I want the recommendation slide tonight. ONE slide. The headline is "recapture ~280bps over 18 months with three actions." Below: the three actions, each with rough sizing of bps impact, the lever (what changes), the owner (who at NorthPeak drives it), and timing. Below that: a small risks & dependencies box. Don't make me read it twice — the CEO needs to nod once and move on.` },
  ],
  artifacts: [
    {
...
const RX_1: Scenario = {
  ...
  opening: [
    { personaId: 'md', text: `Cascade tripped its leverage covenant overnight at 6.2x against a 5.5x test. Apex needs options on the table for the 9am call with the TLB agent. I need a one-page situation overview from you in the next 40 minutes — capital structure, what tripped the covenant, immediate liquidity position, the universe of solutions we'd put on the table. Lead with the answer. Don't give me a history lesson.` },
    { personaId: 'vp', text: `When you're done with the situation overview, build out the cap stack analysis in Excel — current structure with rates, tenors, holders, key covenants, and run a sensitivity showing leverage at +/- 10% EBITDA. I'll need it before the call ends. Get the situation overview to Eleanor first though.` },
  ],
  artifacts: [
    {
...
const RX_2: Scenario = {
  ...
  opening: [
    { personaId: 'principal', text: `Need a clean recovery waterfall for Marquee — three scenarios: liquidation ($185M), going-concern low ($275M), going-concern high ($380M). Walk the priority: admin claims first (assume $15M), then ABL ($120M drawn), then senior secured notes ($250M), then unsecured notes ($100M), then trade ($40M, treated as unsecured), then leases ($80M — treat the cure & assumption costs as priority, assume cure of $25M, rejection damages on the rest as unsecured). Show recovery % by class. For the GC scenarios also show implied recovery in cash vs new equity at a stated exit multiple. I'll review in 35 minutes.` },
  ],
  artifacts: [
    {
...
const RX_3: Scenario = {
  ...
  opening: [
    { personaId: 'sr-assoc', text: `13-week CF for Drayton — DIP sizing conversation tomorrow with the lender. Weekly cadence (W1 through W13). Inflows: customer collections (assume 60% of monthly revenue collects W2-3 after billing, 30% in W4-5, 10% in W6+; build it so the timing matters). Outflows: payroll (every 2 weeks, ~$2.4M each), critical vendor payments (~$5M in W1 then ~$1.5M weekly), other vendors ($800K weekly), restructuring professional fees (~$500K weekly accruing), insurance/utility (~$200K weekly). Starting cash $8M. Minimum operating cash $5M. Tell me the minimum DIP needed to clear all 13 weeks. Build it so a lender can audit it.` },
  ],
  artifacts: [
    {
      id: 'thirteen-week',
...
const ST_1: Scenario = {
  ...
  opening: [
    { personaId: 'sr-trader', text: `Morning. We're long $40M XYZ '32 from the print yesterday, long $25M ABC '29, short $15M HealthCo '30 we need to cover. Curve rallied 6bps overnight on EU PCE. ISM at 8:30, Fed speaker at 10, 30Y at 1pm. Write me a one-paragraph desk commentary for the sales force, and then build me a starting axe sheet for the day — what we're better buyers / sellers of, in what size, at what indicative levels. Send both before 7:45.` },
  ],
  artifacts: [
    {
...
const ST_2: Scenario = {
  ...
  opening: [
    { personaId: 'sr-trader', text: `Long day. Need your EOD note in 25 minutes. What we did today, why, P&L attribution (rates vs spread), positions going into tomorrow, any concerns. Be honest. I'd rather see "we faded the wrong direction on HealthCo and paid for it" than spin.` },
  ],
  artifacts: [
    {
      id: 'eod-note',
...
const ST_3: Scenario = {
  ...
  opening: [
    { personaId: 'sr-trader', text: `Got an RFQ — real-money wants a bid on $20M MidwestTel '30. Last screen G+155 about 90 min ago, comp BBB telecom in the +150-160 zip. We're flat the name. Send me a memo in 10 minutes: your proposed bid (in spread terms), why that level, what the risk looks like if we win it, and whether we have a natural buyer to recycle. I'm on a call but I'll read it on the way back. If the bid makes sense I'll send back "go" and you hit them.` },
  ],
  artifacts: [
    {
      id: 'trade-rationale',
...
const AM_1: Scenario = {
  ...
  opening: [
    { personaId: 'pm', text: `Norden printed. By 7am I need: (1) the model updated — rebase Q4 to the new guide, push the outer years if anything changes, recompute fair value at current multiple and at the multiple you think the stock should trade; (2) a 200-word brief — what changed in your view, what didn't, and your action recommendation (add / hold / trim). Don't recap the press release. Tell me what to do.` },
  ],
  artifacts: [
    {
      id: 'model-update',
...
const AM_2: Scenario = {
  ...
  opening: [
    { personaId: 'pm', text: `BSC is down 11% in two weeks vs sector -3%. No earnings, no news. Two of your channel checks today suggested softer order volumes. Arkema said pricing was tougher than they expected last week and you flagged it. I need a position monitoring brief — is this noise, an early signal, or a real thesis crack? Be honest. If you think we should trim now, say it. Half a page.` },
  ],
  artifacts: [
    {
      id: 'position-brief',
...
const AM_3: Scenario = {
  ...
  opening: [
    { personaId: 'pm', text: `Send me the VIS 1-pager for Thursday's meeting. Standard format. The thesis has to be on one page. The thing I'm going to push on is the multiple — you have to defend why we pay up. Include your action proposal (size, entry plan) and what would kill the trade.` },
  ],
  artifacts: [
    {
      id: 'pitch-onepager',
...
const VC_1: Scenario = {
  ...
  opening: [
    { personaId: 'partner', text: `Lattice Robotics deck just landed. Series A, $18M at $80M pre, warehouse RaaS, ex-Amazon team, $2.4M ARR growing 25% MoM. By lunch I need a triage take — half a page max, your recommendation on whether we take the founder call, what you want to validate on the call. Be opinionated. I'd rather see a strong pass than a fence-sit.` },
  ],
  artifacts: [
    {
      id: 'triage-take',
...
const VC_2: Scenario = {
  ...
  opening: [
    { personaId: 'partner', text: `Memo for Lattice goes in the partner deck Wednesday EOD. Standard format. Real sections — recommendation up top, then thesis, market, team, business, financial profile, deal terms, risks, references. The thing partner meeting will push on is "what's the actual moat" — be ready for it.` },
...
const VC_3: Scenario = {
  ...
  opening: [
    { personaId: 'partner', text: `By Friday I need a market map on legal-tech AI. Structured — segment the landscape, name the players in each segment with a one-line description and rough stage/funding, identify 2-3 white space areas with a hypothesis on why no one has cracked them yet, and end with what you'd be looking to fund. Half-page to one page, but dense. Don't make me read a CB Insights report.` },
  ],
  artifacts: [
    {
      id: 'market-map',
...
const RE_1: Scenario = {
  ...
  opening: [
    { personaId: 'principal', text: `Magnolia Pines is on our desk. 248-unit Class B in Cobb County, $58M ask, $233K/unit. We're underwriting value-add — $14K/unit interior reno, push renovated rents to $1,650 (T-12 average is $1,415). I need a Year 1 - Year 5 cash flow model AND a one-page pre-read by EOD. Inputs: 65% LTV agency at 6.35%, 5-yr IO, 10-yr term, 5-yr hold. Renovate 50% of units in Y1 (10 units/month), 50% in Y2. Standard expense ratios for the submarket. Exit at 5.75% cap on Y5 stabilized NOI. Show levered + unlevered IRR, equity multiple, and Y5 NOI yield on cost.` },
  ],
  artifacts: [
    {
...
const RE_2: Scenario = {
  ...
  opening: [
    { personaId: 'mp', text: `Two issues with the underwrite. One — your 5.75% exit cap is light given where Cobb County has been trading. Recent printed comps are closer to 6.00-6.25. Two — the $235 rent premium relies on comps that are 2-3 quarters stale. Re-run the returns: base, base-at-6.0%-cap, downside-at-6.25%-cap-with-$200-premium. Then rewrite the assumptions section of the memo defending whatever assumption you keep, and being honest about where you've conceded. 35 minutes.` },
...
const RE_3: Scenario = {
  ...
  opening: [
    { personaId: 'sr', text: `Build me a comp set for the West Phoenix / Glendale deal. 14 trades in the universe — pick the 8-10 that are actually comparable to our target (assume 1998-vintage Class B, 220 units, garden-style). For each comp show: address/property, sale date, sale price, $/unit, cap rate, year built, unit count, vintage class. Then summarize implied cap range and $/unit range. At the bottom: which 4-6 you rejected and why. Don't pad it.` },
  ],
  artifacts: [
    {
      id: 'comp-sheet',
...
const ER_1: Scenario = {
  ...
  opening: [
    { personaId: 'sr-analyst', text: `Crestwood printed. Strong overall — $4.85B revenue (2.8% beat), EBITDA beat, EPS beat. Aerospace was the hero (+18% vs +14% expected), Industrial Automation missed. Guide raised. Stock indicated +4% pre-market. By 7:30am I need (1) updated model with Q3 actuals and the raised guide reflected, (2) a 300-word note for clients — call to action, key takes, what changes vs prior view, price target update. Don't recap the press release.` },
  ],
  artifacts: [
    {
      id: 'earnings-model',
...
const ER_2: Scenario = {
  ...
  opening: [
    { personaId: 'sr-analyst', text: `Hartford Capital wants 30 min this afternoon. Their PM is going to push hard on (a) Aerospace growth durability — is 14-16% sustainable through 2026 — and (b) is IA softness cyclical or share loss to MakerEdge and Schneider. Write me a prep doc: 5-7 anticipated questions, your best answer to each, where you'd say "I don't know" honestly, and our house talking points. Don't bullshit — Hartford has been holding CRST for 6 years, they know it better than half our coverage team.` },
  ],
  artifacts: [
    {
      id: 'call-prep-doc',
...
const ER_3: Scenario = {
  ...
  opening: [
    { personaId: 'sr-analyst', text: `Stock is at $156 — up 6.5% on the day. PT is $162. Implied upside is now ~4%. The question is: do we stay Outperform with thin upside, do we upgrade to Buy because conviction is higher post-print, or do we trim to Market Perform because the multiple has re-rated? Write me a rating memo. Take a position. The research director will sign off this afternoon.` },
  ],
  artifacts: [
    {
      id: 'rating-memo',
...
const AU_1: Scenario = {
  ...
  opening: [
    { personaId: 'senior', text: `RR-04 control testing for SignalView. Population is ~840 new subscription contracts Q1-Q3. Test design effectiveness AND operating effectiveness. I need the workpaper documented end-to-end: process narrative, control attributes (what we're actually testing per execution), sample selection rationale (size + method), testing approach for each attribute, and how we'll document results. James is reviewing this — he'll comment on everything if it's not self-contained.` },
    { personaId: 'mgr', text: `When you write the sample size justification: I want the reasoning, not just the number. If you say "60 samples" because that's what the methodology guide says for a population this size, that's fine — but state WHY 60 and not 30 or 90. And if you're going to use any inquiry-only sampling (e.g., judgmental selection), tell me why.` },
  ],
  artifacts: [
    {
...
const AU_2: Scenario = {
  ...
  opening: [
    { personaId: 'senior', text: `Tom is refusing to give you the contract amendments on the 4 exceptions. That's not optional — we have the right to those documents under our engagement letter and PCAOB AS 1105. Write me the memo: what you asked for, what Tom said, why his "policy" reasoning doesn't hold, the implication if we can't get the documents, and your recommended next step. James will take this to the partner if needed. Be professional but clear — this is the moment where junior auditors either hold the line or fold.` },
  ],
  artifacts: [
    {
      id: 'dispute-memo',
...
const AU_3: Scenario = {
  ...
  opening: [
    { personaId: 'mgr', text: `Three review notes on RR-04. (1) Sample under-weights Q3. (2) Tie-out missing on 11 samples. (3) Conclusion language doesn't match what you actually found. I need a response memo: for each of the three notes, your remediation action, what additional work you'll do, and the revised conclusion language. Then update the workpaper accordingly. Don't argue the notes — fix them.` },
  ],
  artifacts: [
    {
      id: 'review-response',
