...
export default function MarketsPage() {
  ...
  const canReply = replyText.trim().length > 0 && replyText.length <= 280 && userId;
  ...
  const handleReplySubmit = async (parentId: Id<"marketPosts">) => {
    if (!canReply) return;
    try {
      await createPost({
        userId: userId as Id<"users">,
  ...
  const { trending, rising } = useMemo(() => {
    ...
    // Only count tags from parent posts to avoid reply-spam gaming the metrics
    rawPosts.filter(p => !p.replyTo).forEach(post => {
    ...
    const sortedRising = [...allTags].filter(t => t.velocity > 0 && t.count <= 3).sort((a,b) => b.velocity - a.velocity).slice(0, 3);
  ...
  // Render a single post node (reusable for parent or child)
  const renderPostNode = (post: PostResponse, isReply: boolean = false) => {
    const sColor = (SENTIMENT_COLORS as any)[post.sentiment] || SENTIMENT_COLORS.Neutral;
    const hasUpvoted = userId && (post.upvotedBy || []).includes(userId);
    const hasDownvoted = userId && (post.downvotedBy || []).includes(userId);
      ...
      <div key={post._id} style={{
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <div className="post-sidebar">
            <div className="post-avatar" style={{ width: isReply ? 32 : 40, height: isReply ? 32 : 40, fontSize: isReply ? 12 : 14 }}>
              {post.author.avatar}
            </div>
            {!isReply && post.replies > 0 && <div className="post-thread-line"></div>}
          </div>
          <div className="post-content-container">
            <div className="post-header">
              <div className="post-author-info">
                ...
                {post.author.badge && (
                  <svg className="verified-badge inline-badge" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
                )}
            ...
            <div className="post-actions">
              <button 
                ...
                style={hasUpvoted ? { color: '#10b981', background: 'rgba(16,185,129,0.1)' } : {}}
                onClick={() => handleAction(post._id, 'upvote')}
              >
                <svg viewBox="0 0 24 24" fill={hasUpvoted ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>
                <span>{post.upvotes}</span>
              </button>
              <button 
                className="action-btn downvote" 
                style={hasDownvoted ? { color: '#ef4444', background: 'rgba(239,68,68,0.1)' } : {}}
                onClick={() => handleAction(post._id, 'downvote')}
              >
                <svg viewBox="0 0 24 24" fill={hasDownvoted ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2h-2"></path></svg>
              ...
              {!isReply && (
                <button 
                  ...
                  style={replyingTo === post._id ? { color: '#3b82f6', background: 'rgba(59,130,246,0.1)' } : {}}
                >
            ...
            {!isReply && replyingTo === post._id && (
              <div style={{ marginTop: '16px', display: 'flex', gap: '12px' }}>
                <textarea 
                  ...
                  style={{ background: 'var(--surface-2)', padding: '12px', borderRadius: '12px', minHeight: '60px' }}
                />
                <button 
                  onClick={() => handleReplySubmit(post._id)}
                  disabled={!canReply}
                  className={`composer-btn ${canReply ? 'active' : ''}`}
            ...
            {/* Recursively load children if any */}
            {!isReply && (
              <div className="replies-wrapper">
                {rawPosts.filter(p => p.replyTo === post._id).sort((a,b) => a.createdAt - b.createdAt).map(child => renderPostNode(child, true))}
              </div>
            )}
          ...
          <div className="markets-feed-col">
            ...
            <div className="markets-composer">
              ...
              <div className="composer-body">
                <textarea 
                  placeholder="Drop a hot take. Add a #hashtag to start a trend."
                  value={composeText}
          ...
          <div className="markets-sidebar-col">
            ...
            <div className="community-guidelines">
              ...
              <ul className="guidelines-list">
                <li>Every post should ideally contain a #hashtag, but it's not required.</li>
